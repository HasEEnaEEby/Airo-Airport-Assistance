...
class SerialThread(threading.Thread):
    def __init__(self):
        ...
        self.command = None
        try:
            self.ser = serial.Serial('/dev/cu.usbmodem11201', 115200, timeout=.025, write_timeout=.025)  # Adjust to your Arduino's serial port
...
def detect_lines(image):
    ...
    if contour is not None:
        ...
        if M["m00"] != 0:
            ...
            print(cx)
            if (45 < cx < 80) or (80 <= cx <= 100):                
                send_wheel_command(-6, -6.9)  # Move forward
            elif cx < 60:
                send_wheel_command(-6.9, -4.5)  # Turn left
            elif cx > 120:
                send_wheel_command(-4.5, -6.9)  # Turn right
...
def decode_qr_code(frame, last_detected_time, cooldown, allow_detection):
    ...
    for qr_code in qr_codes:
        ...
        # Check if the QR code is close enough (e.g., width greater than a threshold)
        if w > 100:  # Adjust this threshold as needed
        ...
        while self.running:
            ...
            if self.stop_robot:
                ...
                if elapsed_time < self.stop_duration or self.stop_duration == float('inf'):
                    ...
                    send_wheel_command(-6.9, 6.9)  # Turn with the specified values
                    time.sleep(2)  # Stop for 2 seconds after turning
                    self.stop_robot = False
            ...
            if not line_detected and not self.stop_robot:
